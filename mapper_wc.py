...
def mapper_wc(p_id, IP, partitionPath, mapPath):#, q, IP):
    ...
    mapped_kv = [(word.lower().strip(),1) for word in data.split() if word.lower().strip() not in stopwords]
